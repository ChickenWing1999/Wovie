    ...
    <div class="modal-body p-5">
        <div class="form-group">
            <label class="custom-label">
                <?php echo __('Movie or TV Series Name');?></label>
            <select name="name" class="form-control selectize-content" data-placeholder="<?php echo __('Movie or TV Series Name');?>" data-ajax="posts">
            </select>
        </div>
    </div>
</div>
        ...
        onChange: function(value) {
            $.ajax({
                ...
                success: function(resp) {
                    var id = $(".videos-accordion").children().length;
                    var movieDetail = [{
                        id: id,
                    ...
                    $('#card-video').tmpl(movieDetail).appendTo('.videos-accordion');
                    $('.modal').modal('hide');
